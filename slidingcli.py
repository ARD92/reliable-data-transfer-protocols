...
while (B==True):
	try:	
		...
		#chkseq=s.recv(1024)
		#print 'chkseq at Transmitter:',chkseq
		cumsum()
		recchk=csum
		print 'received cumulative seq :' ,recchk
		s.sendto('data_check',addr)
		datachk=s.recv(1024)
		if int(datachk)==int(recchk):
			s.sendto('continue',addr)
			se_seq=random.randrange(0,leng_rec) ## sending seq number randomly from rec end.
			print 'sending seq'
			s.sendto(str(se_seq),addr)
			print 'sent seq',se_seq
		else:
			##checking which data hasnt been received,requesting for retransmission of only this data
			datnotrec=int(datachk)-int(recchk)
			s.sendto(str(datanotrec),addr)
			print ('requesting to resend the data containing sequence number:'),datnotrec
s.sendto('exit',addr)
s.close()				
	#except socket.error:
